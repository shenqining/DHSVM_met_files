...
OutputFile = 'E:\DHSVM\grid_data\stationdata\stationdata\pa\tmin\pa_tmin_dat.txt';
OutputFile_stn = 'E:\DHSVM\grid_data\stationdata\stationdata\pa\tmin\pa_tmin_stn.txt';
...
for ii = 1:length(STATION)
    disp(ii);
    if isnan(cell2mat(Cell_station{ii}(1,TMIN_col)))~= 1
        data_output_1 = cell2mat(Cell_station{ii}(1,TMIN_col));
    else
    ...
    for jj = 2:length(Cell_station{ii}(:,1))
        ...
        if pre_date ~= current_date - 1
            ...
            if jj > 1
                data_output_1 = [data_output_1; cell2mat(Cell_station{ii}(jj, TMIN_col))];
...
for aa = 1:length(STATION)
    ...
    data_reformat = int32(data_reformat);
